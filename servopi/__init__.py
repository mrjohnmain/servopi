...
"""
Control Module for Parallax Propeller Servo Controller
...
The signal is a repeated pulse with a variable width in microseconds.
...
class ServoPiError(Exception):
    """ Custom ServoPi Exception. """
...
class controller(object):
    ...
    def __init__(self, port, write_sleep=0.05, controller='PROPELLER_28830',
        ...
        Raises
        ------
        ServoPiError if controller is not implemented.

        """
        if controller == 'PROPELLER_28830':
            self.controller = Propeller28830(port, write_sleep=write_sleep)
        else:
            raise ServoPiError("Invalid controller board.")
        self.write_sleep = write_sleep
        self.debug = debug
        self.enable_all_servos()
        self._debug("Initialized servopi.controller.")
    ...
    def rotate(self, channel, angle, ramp_speed=0):
        self._debug("Rotating channel %d to %d." % (channel, angle))

        pulse_width = int(250 + round(2.7777 * angle));

        self.set_position(channel, pulse_width, ramp_speed)
    ...
    def set_position(self, channel, pulse_width, ramp_speed=0):
        ...
        *WARNING*: the servo may refuse to move at all if the requested
        ...
        Raises
        ------
        ServoPiError for invalid channel or pulse_width values.
        ...
        if channel not in range(9):
            raise ServoPiError("Channel must be between 0 and 8.")
        if pulse_width < self.MIN_PULSE_WIDTH:
            min_pw = self.MIN_PULSE_WIDTH
            msg = ("Pulse width for channel %d "
                   "must be greater than %d.") % (channel, min_pw)
            raise ServoPiError(msg)
        elif pulse_width > self.MAX_PULSE_WIDTH:
            max_pw = self.MAX_PULSE_WIDTH
            msg = ("Pulse width for channel %d "
    ...
    def enable_servo(self, channel):
        ...
        Raises
        ------
        ServoPiError for invalid channel value.
    ...
    def move(self, channels, pulse_widths, duration=0.3):
        ...
        *NOTE*: duration does NOT include the time it takes to read the
        ...
        Raises
        ------
        ServoPiError for invalid channels, pulse_widths or duration values.
